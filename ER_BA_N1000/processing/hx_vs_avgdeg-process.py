...
# Process the data. Store average cross-entropy, average node entropy, and average network statistics
data = np.zeros((8,len(param_list)))
...
df = pd.DataFrame({"%s" % param_sym:param_list, "hx_avg":data[0,:], "hx_std":data[1,:],
                   ...
                   "assortativity":data[6,:],"average_degree":data[7,:]})
df.to_csv("hx_%s.csv" % graph, index=False)
