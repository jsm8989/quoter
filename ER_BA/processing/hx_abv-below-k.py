...
def get_data(kind="above"):
    ...
    for i,x in enumerate(param_list):
        ...
        for trial in trials_list:
            nfile = datadir + "node/" + "N%i_m%i_q%0.1f_T%i_sim%i.txt" % (N,x,q,T,trial)
            efile = datadir + "edge/" + "N%i_m%i_q%0.1f_T%i_sim%i.txt" % (N,x,q,T,trial)
            gfile = datadir + "graph/" + "N%i_m%i_q%0.1f_T%i_sim%i.txt" % (N,x,q,T,trial)
            if os.path.isfile(nfile) and os.path.isfile(efile) and os.path.isfile(gfile):  
                ...
                k = gdata["average_degree"].values[0]
##                C = np.mean(ndata["C"].values)
                
                for ego in egos:
                    ego_deg = ndata["indegree"].loc[ndata["node"] == ego].iloc[0]
...
abv = get_data("above")
blw = get_data("below")
df=pd.DataFrame({"above_average_k": abv, "below_average_k": blw})
df = df.to_csv('hx_abv-blw-k_BA.csv', index=False)
