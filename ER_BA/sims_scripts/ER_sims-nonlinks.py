...
##    NUMJOBS = 1
    ...
    edgelists = next(os.walk("../data_ER/edge/"))[2]
    ...
    for filenum in params:
        elist = next(os.walk("../data_ER/edge/"))[2][filenum]
        outdir = "../data_ER-nonlinks/"
        outfile = elist
        if not os.path.isfile(os.path.join(outdir, outfile)): # avoid re-doing & overwriting
            G = read_edgelist(os.path.join("../data_ER/edge/",elist)).to_directed()
