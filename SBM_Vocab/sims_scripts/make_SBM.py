...
def make_growing_SBM(N,p,mu_seq,trial):
    ...
    G.add_edges_from(eb + ew)
    nx.write_edgelist(G,"edgelist/N%i_p%0.2f_mu%0.4f_trial%i.txt" % (N,p,mu_seq[0],trial), delimiter=" ", data=False)
    
    for i in range(1,len(mu_seq)):
        edges_to_add = int( (mu_seq[i] - mu_seq[i-1])*m**2 )
        eb = itertools.product(A,B)
        eb_remaining = list(nx.non_edges(G))
        eb_remaining = (set(eb_remaining) | set([x[::-1] for x in eb_remaining])) & set(eb)
        e = random.sample(eb_remaining, edges_to_add)
        G.add_edges_from(e)
        nx.write_edgelist(G,"edgelist/N%i_p%0.2f_mu%0.4f_trial%i.txt" % (N,p,mu_seq[i],trial), delimiter=" ", data=False)
