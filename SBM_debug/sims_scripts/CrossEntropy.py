...
@jit(parallel=True)
def get_all_lambdas_fix(target, source, relative_pos, lambdas):
    """ 
    Finds all the the longest subsequences of the target, 
    that are contained in the sequence of the source,
    with the source cutoff at the location set in relative_pos.
...
def timeseries_cross_entropy_fix(time_tweets_target, time_tweets_source, please_sanitize = True, get_lambdas = False):
    """ 
    Finds the cross entropy H_cross(target|source) where the processes are embedded in time.
    ...
    lambdas = get_all_lambdas_fix(target ,source, relative_pos, lambdas)
