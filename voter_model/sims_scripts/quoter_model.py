...
def quoter_model_sim(G,q,T,outdir,outfile,write_data=write_all_data):
    ...
    for node in G.nodes():
            newWords = np.random.choice(vocab, size=1, replace=True, p=weights).tolist()
            G.node[node]["words"] = newWords
            G.node[node]["times"] = [0]

    # spins_up = [sum([G.node[x]["words"][-1] for x in G.nodes()])]
    ...
    for t in range(1,T*nx.number_of_nodes(G)):
            ...
            if random.random() < q and len(nbrs) > 0:
                    ...
                    newWords = words_friend[-1]
                
            else:
                    newWords = 1-G.node[node]["words"][-1] # flip state
##                        newWords = np.random.choice(vocab, size=1, replace=True, p=weights) # pick next state randomly

                    

            G.node[node]["words"].append(newWords)
            G.node[node]["times"].append(t)

            # spins_up.append(sum([G.node[x]["words"][-1] for x in G.nodes()]))
