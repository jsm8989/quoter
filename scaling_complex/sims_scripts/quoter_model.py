...
def quoter_model_sim(G,q,T,outdir,outfile,write_data=write_all_data,dunbar=None):
    ...
    for t in range(1,T*nx.number_of_nodes(G)):
        ...
        if random.random() < q and len(nbrs) > 0:
            ...
            if tweetLength > 0: # NEED TO DEAL WITH THIS? POISSON CAN TAKE ON ZERO
                if "quote_times" in G.node[node]:
                    G.node[node]["quote_times"].append(t)
                else:
                    G.node[node]["quote_times"] = [t]
