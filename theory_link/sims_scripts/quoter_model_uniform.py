...
def new_content(z,n):
    # uniform vocabulary distribution
    words = np.random.choice(list(range(1,z+1)), size=n, replace=True).tolist()
    return words
...
def quoter_model_sim(G,q,T,lam,outdir,outfile,z_alter,z_ego,write_data=write_all_data):
    ...
    for node in G.nodes():
        if node == 1:
            newWords = new_content(z_ego,startWords)
        else:
            newWords = new_content(z_alter,startWords)
    ...
    for t in range(1,T*nx.number_of_nodes(G)):
        ...
        if random.random() < q and len(nbrs) > 0:
            ...
            copy_pos_start = random.choice( list(range(max( 1, numWords_friend-tweetLength))) )
            copy_pos_end = min( numWords_friend-1, copy_pos_start + tweetLength)
            newWords = words_friend[copy_pos_start: copy_pos_end]
            
        else: # new content
            if node == 1:
                newWords = new_content(z_ego,tweetLength)
            else:
                newWords = new_content(z_alter,tweetLength)
